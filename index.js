...
async function is_guestup_path(path) {
    let guestup_path = await getConfig('guestup_path', SERVER['disktag']);
    path = path_format(path);
    guestup_path = path_format(guestup_path);
    if (path.substr(-1) == '/') path = path.substr(0, path.length - 1);
    if (guestup_path.substr(-1) == '/') guestup_path = guestup_path.substr(0, guestup_path.length - 1);
    return path === guestup_path;
}
